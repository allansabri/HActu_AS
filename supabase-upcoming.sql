create table if not exists public.upcoming_releases (
  ...
  type text not null default 'series' check (type in ('movie', 'series', 'documentary', 'special', 'sport')),
  synopsis text,
  release_label text,
  release_date date,
...
alter table public.upcoming_releases add column if not exists release_time text;
